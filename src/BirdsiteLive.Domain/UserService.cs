...
public class UserService : IUserService
{
    ...
    private async Task<bool> SendAcceptFollowAsync(ActivityFollow activity, string followerHost)
    {
        ...
        HttpStatusCode result = await _activityPubService.PostDataAsync(acceptFollow, followerHost, activity.apObject);

        //TODO: revamp this for better error handling
        return result is HttpStatusCode.Accepted or HttpStatusCode.OK;
    }
    ...
    private static string OnlyKeepRoute(string inbox, string host)
    {
    ...
    private async Task<SignatureValidationResult> ValidateSignature(string actor, string rawSig, string method,
        string path, string queryString, IReadOnlyDictionary<string, string> requestHeaders, string body)
    {
        //Check Date Validity
        ...
        // Trust and Verify
        bool result = signKey.VerifyData(Encoding.UTF8.GetBytes(toSign.ToString()), sig, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
