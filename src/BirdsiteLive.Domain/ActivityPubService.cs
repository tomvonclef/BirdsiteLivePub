...
public class ActivityPubService : IActivityPubService
{
    ...
    public async Task<Actor> GetUser(string objectId)
    {
        ...
        Debug.Assert(actor != null, nameof(actor) + " != null");
    ...
    public async Task PostNewNoteActivity(Note note, string username, string noteId, string targetHost, string targetInbox)
    {
        try
        {
            string actor = UrlFactory.GetActorUrl(_instanceSettings.Domain, username);
            string noteUri = UrlFactory.GetNoteUrl(_instanceSettings.Domain, username, noteId);

            var now = DateTime.UtcNow;
            string nowString = now.ToString("s") + "Z";
