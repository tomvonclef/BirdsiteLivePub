...
public class RetrieveTwitterUsersProcessor : IRetrieveTwitterUsersProcessor
{
    ...
    public int WaitFactor = 60_000; // 1 min
    ...
    public async Task GetTwitterUsersAsync(BufferBlock<SyncTwitterUser[]> twitterUsersBufferBlock, CancellationToken ct)
    {
        for (; ; )
        {
            ...
            try
            {
                int maxUsersNumber = await _maxUsersNumberProvider.GetMaxUsersNumberAsync();
                var users = await _twitterUserDal.GetAllTwitterUsersAsync(maxUsersNumber);

                int userCount = users.Any() ? users.Length : 1;
                int splitNumber = (int) Math.Ceiling(userCount / 15d);
                ...
                int splitCount = splitUsers.Count;
                if (splitCount < 15)
