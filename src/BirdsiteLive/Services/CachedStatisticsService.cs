...
public class CachedStatisticsService : ICachedStatisticsService
{
    ...
    public async Task<CachedStatistics> GetStatisticsAsync()
    {
        if (_cachedStatistics != null &&
            !((DateTime.UtcNow - _cachedStatistics.RefreshedTime).TotalMinutes > 15)) return _cachedStatistics;

        int twitterUserMax = _instanceSettings.MaxUsersCapacity;
        int twitterUserCount = await _twitterUserDal.GetTwitterUsersCountAsync();
        int saturation = (int)((double)twitterUserCount / twitterUserMax * 100);
